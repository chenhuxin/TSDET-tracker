function [net,stats] = cnn_train_dagnn(net, imdb, input, getBatch, varargin)
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    inputs ={'input1',gpuArray(ims),'input2', ...
        gpuArray(img), 'label', gpuArray(labels)};
  ...
  for f = setdiff(fieldnames(stats)', {'num', 'time'})%setdiff(A,B)返回在A中有，而B中没有的值，结果向量将以升序排序返回。在集合论中，c = A - B。A和B也可以是字符串细胞数组。
    f = char(f) ;
