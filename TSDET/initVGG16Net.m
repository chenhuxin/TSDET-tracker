...
net2.layers(31:end) = [];%删除全连接层

avgImg=net2.meta.normalization.averageImage;

net1=dagnn.DagNN();%初始化dagnn

net1.addLayer('conv1_1', dagnn.Conv('size', [3,3,3,64],...
    'hasBias', true, 'pad',...
    [1,1,1,1], 'stride', [1,1]), 'input', 'conv_11', {'conv11_f', 'conv11_b'});%addlayer(layername,layer,input,output,params)
net1.addLayer('relu1_1', dagnn.ReLU(),'conv_11','relu_11');

f = net1.getParamIndex('conv11_f') ;%获取参数索引
...
% net1.addLayer('pool3', dagnn.Pooling('poolSize', [2,2], 'pad',...
%     [0,1,0,1], 'stride', [2,2]),'relu_33','pool_3');
...
net1.params(f).weightDecay=1;
%---------------------conv5-----------------
net1.addLayer('conv5_1', dagnn.Conv('size', [3,3,512,512],...
    'hasBias', true, 'pad',...
    [1,1,1,1], 'stride', [1,1]), 'relu_43', 'conv_51', {'conv51_f', 'conv51_b'});
net1.addLayer('relu5_1', dagnn.ReLU(),'conv_51','relu_51');

f = net1.getParamIndex('conv51_f') ;
net1.params(f).value=net2.layers{25}.weights{1};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1e3;

f = net1.getParamIndex('conv51_b') ;
net1.params(f).value=net2.layers{25}.weights{2};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1;

net1.addLayer('conv5_2', dagnn.Conv('size', [3,3,512,512],...
    'hasBias', true, 'pad',...
    [1,1,1,1], 'stride', [1,1]), 'relu_51', 'conv_52', {'conv52_f', 'conv52_b'});
net1.addLayer('relu5_2', dagnn.ReLU(),'conv_52','relu_52');

f = net1.getParamIndex('conv52_f') ;
net1.params(f).value=net2.layers{27}.weights{1};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1e3;

f = net1.getParamIndex('conv52_b') ;
net1.params(f).value=net2.layers{27}.weights{2};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1;

net1.addLayer('conv5_3', dagnn.Conv('size', [3,3,512,512],...
    'hasBias', true, 'pad',...
    [1,1,1,1], 'stride', [1,1]), 'relu_52', 'conv_53', {'conv53_f', 'conv53_b'});

f = net1.getParamIndex('conv53_f') ;
net1.params(f).value=net2.layers{29}.weights{1};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1e3;

f = net1.getParamIndex('conv53_b') ;
net1.params(f).value=net2.layers{29}.weights{2};
net1.params(f).learningRate=0;
net1.params(f).weightDecay=1;
